...
const TimeEntryList = ({ userId, date, limit }: TimeEntryListProps) => {
        ...
        <div key={entry.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
            ...
            {user?.role === 'admin' && !userId && (
              <div className="text-sm text-gray-500">{getUserName(entry.userId)}</div>
            )}
          </div>
            ...
            <div className="flex justify-between mb-3">
              ...
              <div className="flex items-center">
                ...
                <span className="ml-2 text-sm">
                  {entry.clockOut || (entry.status === 'active' ? 'Active' : 'Not recorded')}
                </span>
              </div>
            </div>
              ...
              <div className="mt-3">
                ...
                <div className="pl-6 space-y-1">
                  {entry.breaks.map(breakItem => (
                    <div key={breakItem.id} className="text-sm flex justify-between">
                      <div>
            ...
            {entry.totalHours !== null && (
              <div className="mt-4 pt-3 border-t border-gray-100 flex justify-between items-center">
                ...
                <span className="text-sm font-semibold">
                  {Math.floor(entry.totalHours)}h {Math.round((entry.totalHours % 1) * 60)}m
                </span>
              </div>
            )}
          </div>
